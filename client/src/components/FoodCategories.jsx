  ...
  return (
    <div>
      <input 
        ...
        required 
      />
      <button onClick={search} className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Search</button>
      <div>
        {searched && <FilterFood items={categories.meals}/>}
      </div>
      <div className="grid md:grid-cols-4 grid-cols-1 m-4">
        {
          categories.meals && categories.meals.map((item)=>{
            return (
              <div key={item.idMeal} className="p-4" style={{visibility: filtered ? 'hidden' : 'visible'}}>
